...
class PayPal extends React.Component {
    render() {
        const onSuccess = (payment) => {
            ...
            let time = new Date();
            axios.post("/createOrder", { order: this.props.orderCakeReducer, info: this.props.orderFormReducer, time: `${time}` }).then((result) => {
        ...
        const onCancel = (data) => {
            // User pressed "cancel" or closed Paypal popup
            console.log('The payment was cancelled!', data);


            // **TEST** submit order on cancel for testing purposes
            let time = new Date();
            axios.post("/createOrder", { order: this.props.orderCakeReducer, info: this.props.orderFormReducer, time: `${time}` }).then((result) => {
                alert(result.data.message);
            }).catch((error) => {
        ...
        let env = 'production'; // you can set here to 'production' for production, 'sandbox' for sandbox mode
        let currency = 'USD'; // or you can set this value from your props or state  
        let total = 20; // same as above, this is the total amount (based on currency) to be paid by using Paypal express checkout (in dollars)
